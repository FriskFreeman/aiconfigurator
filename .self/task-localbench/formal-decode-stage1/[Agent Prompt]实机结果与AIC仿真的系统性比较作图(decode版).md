...
工作文件夹和结果输出目录：bench_data/h100_sxm/sglang/v0.5.9/deepseek_v3_mla/analysis/mla_aic_compare_decode

## 1. 以AIC的普通DeepSeekModel作对比（kernel时间累加）

- 实机数据和比较口径：每实例每层为一行数据，考虑其GPU makespan时间（首尾kernel包络）和kernel_time_sum时间（所有kernel加总的净时间）。并且详细记录每个kernel的具体持续时间。
  - 由于对于decode开启了cudagraph，从用例输出的csv中没法直接看出层间的边界，因此需要以算子信息自行推断（每层的MLA模块一般以一个qkv_a_proj的quant+gemm开始，以一个q_o_proj的gemm结束）。
  - 在sglang中，decode开启cudagraph时，在qkv_a_proj之后和q_b_proj之前的两个RMSNorm算子（q和k的）设计了一个graph overlap机制，因此这两个norm算子的时间总会有重叠。在kernel_time_sum时需取二者的最大边界算作“双norm总时间”
...
- \*注意：WideEPDeepSeekModel源码中存在一个重复的qkv_a_proj GEMM（hidden -> 2112），为目前bug。比较时仅纳入一个相应的gemm即可。
- 作图：
  - 实机数据侧：与1.一致。
