...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-root", default=".self/task-localbench/tmp/tmp-prefill")
...
def main() -> int:
    ...
    helper_script = Path(__file__).resolve().parent / "run_prefill_inside_container.py"
    run_root = root_dir / args.run_root
    if run_root.name != "output":
        run_root = run_root / "output"
    run_stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = run_root / (
        f"{run_stamp}_sglang_prefill_instance_dummy5_"
        f"cg_off_pcg_{args.pcg_mode}_backend_{args.attention_backend}"
    )
    ...
    docker_cmd = [
        ...
        "-v",
        f"{helper_script}:/runner/run_prefill_inside_container.py:ro",
        args.image,
        "python",
        "/runner/run_prefill_inside_container.py",
        "--model-path",
        "/model",
        "--output-dir",
        "/out",
