...
上面的主比较框架在 0.9.0 仍然大体成立，但有三点新变化值得补充：

### 4.1 三个 backend 都接入了更显式的 per-op 来源追踪

现在在 `run_agg` 和静态评估链路里，不只是记录每个步骤的时延拆分，还会并行记录这些拆分来自：
...
所以 backend 不再只是“算时间”，也在参与“解释这些时间来自哪里”。

### 4.2 三个 backend 都出现了 Rust engine-step 快路径
...
都开始根据 `runtime_config.engine_step_backend` 选择是否走 Rust 估算器来计算：

- mixed step latency
- decode-only step latency
- 静态前向分解

因此相同 backend 的仿真现在也可能有 Python / Rust 两条底层估算路径。

### 4.3 vLLM 的显存记账仍然保守地复用 TRT-LLM 路径

这一点在 0.9.0 依旧没有根本变化。

也就是说：

- 调度仿真上，vLLM 有自己的 `run_agg` 逻辑
- 但显存估算上，当前实现仍然走保守的 TRT-LLM `_get_memory_usage`

这对于理解“为什么 vLLM 结果可能更早触发 OOM 过滤”仍然很关键。
