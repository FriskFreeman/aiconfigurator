...
def _resolve_parameters(
    ...
    if params is not None:
        if normalized != "standard":
            raise ValueError(
                "custom params cannot be combined with a non-standard parameter_level"
            )
...
def bmm_work_terms(
    ...
    return {
        ...
        "scope": (
            "calibrated_deepseek_matrix_geometry"
            if calibrated_op is not None
            else "generic_shape_extrapolation"
        ),
