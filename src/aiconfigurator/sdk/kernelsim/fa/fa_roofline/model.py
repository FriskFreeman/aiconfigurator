...
def _tile_work(shape: AttentionShape, br: int, bc: int) -> dict[str, int]:
    ...
    for query_start in range(0, shape.query_length, br):
        ...
        if shape.causal:
            exact_columns = min(
                shape.kv_length_total, history + query_start + rows
            )
            exact_valid_scores += sum(
                min(shape.kv_length_total, history + row + 1)
                for row in range(query_start, query_start + rows)
            )
        else:
...
def _decode_splits(
    ...
    if isinstance(options.decode_splits, int):
        ...
        shape.query_length > options.decode_query_threshold
        or shape.history_length == 0
    ):
        selected = 1
...
def estimate_attention(
    ...
    kv_splits, split_metadata = _decode_splits(
        hardware, shape, options, query_tiles, bc
    )
    ...
    reduction_parallel_efficiency = max(
        reduction_parallel_efficiency, 1.0 / hardware.sm_count
    )

    score_elements = (
        shape.batch_size
        * shape.query_heads
        * tile_work["score_elements_per_query_head"]
    )
    exact_scores = (
        shape.batch_size
        * shape.query_heads
        * tile_work["exact_valid_scores_per_query_head"]
    )
    ...
    row_updates = (
        shape.batch_size
        * shape.query_heads
        * tile_work["row_tile_updates_per_query_head"]
    )
    ...
    final_normalize_flops = (
        kv_splits * query_rows * (exp_cost + 2 + shape.value_head_dim)
    )
    if kv_splits > 1:
        split_reduction_flops = query_rows * (
            2 * (kv_splits - 1)
            + kv_splits * (exp_cost + 1)
            + exp_cost
            + 1
            + 2 * kv_splits * shape.value_head_dim
        )
    else:
    ...
    query_elements = (
        shape.batch_size * shape.query_heads * shape.query_length * shape.head_dim
    )
    output_elements = (
        shape.batch_size
        * shape.query_heads
    ...
    kv_hbm_heads = (
        shape.kv_heads if options.assume_gqa_hbm_reuse else shape.query_heads
    )
    kv_loaded = tile_work["kv_tokens_loaded_per_query_head"]
    kv_hbm_loaded = (
        shape.kv_length_total
        if options.assume_query_tile_l2_reuse
        else kv_loaded
    )
    kv_l2_elements = (
        shape.batch_size * kv_l2_heads * kv_loaded * shape.kv_storage_dim
    )
    kv_hbm_elements = (
        shape.batch_size
        * kv_hbm_heads
        * kv_hbm_loaded
        * shape.kv_storage_dim
    )
    ...
    partial_bytes = (
        kv_splits * query_rows * (shape.value_head_dim + 1) * 4
        if kv_splits > 1
        else 0
    )
    if kv_splits > 1:
    ...
    live_kv_cache_update_bytes = (
        2 * live_kv_elements * input_bytes
        if options.include_kv_cache_update
        else 0
    )
    ...
    reduction_hbm_bandwidth = (
        hardware.hbm_bandwidth_bytes_s * reduction_parallel_efficiency
    )
    reduction_l2_bandwidth = (
        hardware.l2_bandwidth_bytes_s * reduction_parallel_efficiency
    )
    ...
    raw_l2_s = (
        mainloop_l2_bytes / l2_effective_bandwidth
        + reduction_l2_bytes / reduction_l2_bandwidth
    )
    matrix_s = matrix_flops / matrix_effective_peak
    mainloop_vector_s = mainloop_vector_flops / vector_effective_peak
    boundary_vector_s = (
        split_boundary_flops / vector_effective_peak
        + split_reduction_flops
    ...
    compute_s = (
        matrix_s
        + mainloop_vector_s
        - overlap * min(matrix_s, mainloop_vector_s)
        + boundary_vector_s
    )
    ...
    decode_service_active = (
        shape.history_length > 0
        and shape.query_length <= options.decode_query_threshold
    )
    n_task = (
        shape.batch_size * shape.kv_heads * query_tiles * kv_splits
    ...
    if options.mode == "profiled":
        ...
        if decode_service_active:
            ...
            task_service_s = (
                fractional_waves * effective_task_cycles / hardware.clock_hz
            )
    ...
    work = {
        ...
        "arithmetic_intensity_flop_per_hbm_byte": (
            total_flops / total_counted_hbm_bytes
            if total_counted_hbm_bytes
            else None
        ),
    }
    resources_us = {
