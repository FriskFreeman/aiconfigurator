...
class HardwareSpec:
    ...
    def __post_init__(self) -> None:
        if self.schema_version != SCHEMA_VERSION:
            raise ValueError(
                f"hardware schema_version must be {SCHEMA_VERSION}, got {self.schema_version}"
            )
        ...
        for dtype, peak in self.matrix_peak_flops.items():
            ...
            normalized[canonical] = _positive_number(
                f"matrix_peak_flops[{dtype!r}]", peak
            )
    ...
    def matrix_peak(self, dtype: str) -> float:
        ...
        try:
            ...
            raise ValueError(
                f"hardware has no dense matrix peak for {canonical}; "
                f"configured precisions: {supported}"
            ) from error
...
class AttentionShape:
    ...
    def __post_init__(self) -> None:
        ...
        storage_dim = (
            self.head_dim + value_dim
            if self.kv_storage_dim is None
            else self.kv_storage_dim
        )
