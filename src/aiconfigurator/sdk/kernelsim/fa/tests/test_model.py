...
class FlashAttentionModelTests(unittest.TestCase):
    ...
    def test_h100_decode_split_and_task_count(self) -> None:
        ...
        self.assertEqual(
            result.scheduling["cta_count_hq_mapping_with_splits"], 160
        )
        self.assertEqual(result.scheduling["n_task_b_hkv_query_tiles_splits"], 40)
        self.assertAlmostEqual(
            result.scheduling["fractional_task_waves"], 40 / 132, places=15
        )

    def test_large_batch_short_decode_exposes_service_cost(self) -> None:
        shape = AttentionShape(512, 1, 8, 32, 8, 128, "fp8")
        raw = estimate_attention(
            self.h100, shape, ModelOptions(mode="analytical")
        )
        profiled = estimate_attention(
            self.h100, shape, ModelOptions(mode="profiled")
        )
        self.assertEqual(profiled.tiles["kv_splits"], 1)
        self.assertEqual(
            profiled.scheduling["n_task_b_hkv_query_tiles_splits"], 4096
        )
    ...
    def test_fa3_overlap_never_increases_compute_time(self) -> None:
        ...
        fa2 = estimate_attention(
            self.h100, shape, ModelOptions(algorithm="fa2", mode="analytical")
        )
        fa3 = estimate_attention(
            self.h100, shape, ModelOptions(algorithm="fa3", mode="analytical")
        )
