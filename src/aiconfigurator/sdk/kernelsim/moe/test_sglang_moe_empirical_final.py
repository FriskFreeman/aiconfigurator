...
import sglang_moe_empirical_final as model
...
class SglangMoeEmpiricalFinalTests(unittest.TestCase):
    ...
    def test_ep_scope_and_ideal_work_scaling_are_explicit(self) -> None:
        ep1 = model.estimate_sglang_moe(
            "bf16_triton", 1024, 7168, 2048, 8, 256, 1, 1, 989e12, 3.35e12
        )
        ep8 = model.estimate_sglang_moe(
            "bf16_triton", 1024, 7168, 2048, 8, 256, 1, 8, 989e12, 3.35e12
        )
    ...
    def test_custom_parameters_and_validation(self) -> None:
        ...
        result = model.estimate_sglang_moe(
            "bf16_triton", 8, 16, 32, 2, 4, 1, 1, 1e15, 3e12, custom
        )
        ...
        with self.assertRaisesRegex(ValueError, "peak_flops_s"):
            model.estimate_sglang_moe(
                "bf16_triton", 8, 16, 32, 2, 4, 1, 1, 0.0, 3e12
            )
