...
def work_terms(
    ...
    if recipe in {"bf16_triton", "fp8_block_triton"}:
        ...
        terms["bytes_combine"] = (
            0.0 if routed == 1 else 2.0 * assignments * h + 2.0 * t * h
        )
        ...
        terms.update(
            {
                ...
                "bytes_gemm1_weight_scale": 4.0
                * active
                * _ceil_div(2.0 * j, 128)
                * h_blocks,
                "bytes_gemm1_output": 4.0 * assignments * j,
                "bytes_activation_quant": 9.0 * assignments * j
                + 4.0 * assignments * j_blocks,
                "bytes_gemm2_input": assignments * j + 4.0 * assignments * j_blocks,
                "bytes_gemm2_weight_values": active * h * j,
                "bytes_gemm2_weight_scale": 4.0
                * active
        ...
        terms.update(
            {
                ...
                "bytes_gemm1_output": 4.0 * assignments * j,
                "bytes_activation_quant": 4.5 * assignments * j
                + assignments * j_scale_cols,
    ...
    terms["bytes_total"] = sum(
        float(value) for key, value in terms.items() if key.startswith("bytes_")
    )
    return terms
...
def estimate_sglang_moe(
    ...
    *,
    ...
    weight_value_bytes = float(terms["bytes_gemm1_weight_values"]) + float(
        terms["bytes_gemm2_weight_values"]
    )
    weight_scale_bytes = float(terms["bytes_gemm1_weight_scale"]) + float(
        terms["bytes_gemm2_weight_scale"]
    )
