...
def setup_logging(scope=["all"], debug=False, worker_id=None):
    ...
    # Create log directory. Allow callers such as collect.py to pin an
    # explicit output directory via COLLECTOR_LOG_DIR instead of always
    # generating a timestamped run folder.
    explicit_log_dir = os.environ.get("COLLECTOR_LOG_DIR", "").strip()
    if explicit_log_dir:
        _LOG_DIR = Path(explicit_log_dir).expanduser()
    else:
...
def log_perf(
    ...
    try:
        with open(perf_filename, "a", newline="") as f:
            ...
            # Keep append-only text files schema-stable when new columns are
            # introduced later by writing the union of all keys that appear in
            # the current batch.
            fieldnames = list(base_data.keys()) + [key for key in item_keys if key not in base_data]
            if power_stats:
            ...
            if not is_empty:
                with open(perf_filename, encoding="utf-8", newline="") as existing_f:
