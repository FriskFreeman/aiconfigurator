...
def _resolve_run_output_dir() -> str:
    """Return the active output directory for logs, summaries, and perf txts."""
    log_dir = os.environ.get("COLLECTOR_LOG_DIR", "").strip()
    if not log_dir:
        return os.getcwd()
...
class ProfilerContext:
    ...
    def __enter__(self):
        if self.enabled:
            ...
            self.log_dir = _resolve_run_output_dir()
...
def parallel_run(tasks, func, num_processes, module_name="unknown", resume_options=None):
    ...
    # Log summary
    if errors:
        log_dir = _resolve_run_output_dir()
        logger.error(f"{module_name}: Completed with {len(errors)} errors")
        os.makedirs(log_dir, exist_ok=True)
        error_file = os.path.join(log_dir, f"errors_{module_name}.json")
...
def generate_collection_summary(all_errors, backend, version):
    ...
    # Save summary
    summary_file = os.path.join(log_dir, f"collection_summary_{backend}.json")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory for perf txt outputs plus collection logs/summary/error files. "
        "Defaults to the collector's timestamped run directory.",
    )
    parser.add_argument(
    ...
    if args.output_dir:
        ...
        os.environ["COLLECTOR_LOG_DIR"] = output_dir
    ...
    if args.output_dir:
        logger.info(f"Output directory override active: {os.environ['COLLECTOR_LOG_DIR']}")
