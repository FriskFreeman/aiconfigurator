...
def _is_sm120_triton_flash_mla_enabled() -> bool:
    return os.environ.get(_SM120_FLASHMLA_ENV, "1").strip().lower() not in {"0", "false", "off", "never"}


def _sm120_flash_mla_fallback_label() -> str:
    if not _sglang_is_sm120():
        return ""
    return "sm120_triton" if _is_sm120_triton_flash_mla_enabled() else "sm120_torch"


def _sm120_paged_mqa_backend_label() -> str:
    if not _sglang_is_sm120():
        return ""
    try:
...
def _import_dsv4_sm120_paged_mqa_impl() -> tuple[Callable, Callable]:
    from sglang.srt.environ import envs
    from sglang.srt.layers.attention.dsv4.indexer import fp8_paged_mqa_logits_torch, fp8_paged_mqa_logits_torch_sm120

    if envs.SGLANG_OPT_USE_TILELANG_INDEXER.get():
        from sglang.srt.layers.attention.dsv4.tilelang_kernel import tilelang_fp8_paged_mqa_logits

        return tilelang_fp8_paged_mqa_logits, tilelang_fp8_paged_mqa_logits

    if _sglang_is_sm120():
        return fp8_paged_mqa_logits_torch_sm120, fp8_paged_mqa_logits_torch_sm120

    return fp8_paged_mqa_logits_torch, fp8_paged_mqa_logits_torch


def _import_sm120_flash_mla_impl() -> tuple[Callable, str]:
    if _has_module("sglang.srt.layers.attention.flash_mla_sm120"):
        from sglang.srt.layers.attention.flash_mla_sm120 import flash_mla_with_kvcache_sm120

        return flash_mla_with_kvcache_sm120, _sm120_flash_mla_fallback_label()

    if _has_module("sglang.srt.layers.attention.flash_mla_sm120_fallback"):
        from sglang.srt.layers.attention.flash_mla_sm120_fallback import (
            flash_mla_with_kvcache_entrypoint,
        )

        return (
            lambda **kwargs: flash_mla_with_kvcache_entrypoint(backend="kernel", **kwargs),
            "sm120_fallback_kernel",
        )

    raise ImportError("SGLang SM120 FlashMLA implementation is not available")
...
def _bench_paged_mqa_logits(M: int, past_kv: int, *, batch_size: int = 1, device: str = "cuda:0") -> float:  # noqa: N803
    ...
    fallback = ""
    if _sglang_is_sm120():
        fp8_paged_mqa_logits, get_paged_mqa_logits_metadata = _import_dsv4_sm120_paged_mqa_impl()
        fallback = _sm120_paged_mqa_backend_label()
    else:
    ...
    result["backend"] = fallback or "deep_gemm"
...
def _bench_flash_mla_sparse(
    ...
    *,
    ...
    """
    if _sglang_is_sm120():
        flash_mla_with_kvcache, backend = _import_sm120_flash_mla_impl()
        sched_meta = None
    else:
        from flash_mla import flash_mla_with_kvcache, get_mla_metadata
        backend = "flash_mla_cuda"
    ...
    if _sglang_is_sm120():
        # PR #24692's SM120 FlashMLA path is Triton-autotuned by default and
        # the pure-torch fallback still benefits from a real prewarm before
        # graph capture.  Run the exact benchmark closure so the replayed
        # measurement reflects steady-state execution.
        for _ in range(2):
            kernel_fn()
            torch.cuda.synchronize()
    ...
    result["fallback"] = "" if backend == "flash_mla_cuda" else backend
    return result
...
def run_dsv4_sparse_kernel_worker(
    ...
    *,
    ...
    fallback = bench_result.get("fallback", "")
